...
class TestAccountModel:
    ...
    # << NUEVOS TESTS PARA validate() >>
    def test_validate_success(self):
        """No debe lanzar excepción con datos válidos."""
        account = Account(name="Valido", email="valido@example.com")
        # validate() no retorna, solo debe completar sin excepción
        account.validate()

    def test_validate_empty_name_raises(self):
        """Debe lanzar DataValidationError si name está vacío."""
        account = Account(name="", email="valido@example.com")
        with pytest.raises(DataValidationError) as excinfo:
            account.validate()
        assert "El nombre no puede estar vacío" in str(excinfo.value)

    def test_validate_invalid_email_raises(self):
        """Debe lanzar DataValidationError si el email no es válido."""
        account = Account(name="Usuario", email="no-valido")
        with pytest.raises(DataValidationError) as excinfo:
            account.validate()
